...
async def send_message(request: ChatMessageRequest):
    ...
    try:
        ...
        if request.session_id:
            await user_stats_service.update_user_activity(request.session_id)
...
async def create_new_session():
    """إنشاء جلسة محادثة جديدة مع تتبع المستخدم"""
    try:
        session_id = await chat_service._create_new_session()
        
        # تتبع المستخدم الجديد
        await user_stats_service.track_user_session(session_id)
...
# ...
# بعد كل أكواد FastAPI والاستيرادات الأخرى
# ...

# -----------------------------------------------------------------
# نقطة الدخول الجديدة: Mangum
# -----------------------------------------------------------------
from mangum import Mangum

# يجب أن يكون 'app' هو اسم متغير تطبيق FastAPI المُعرَّف في مكان ما أعلى الملف
handler = Mangum(app) 